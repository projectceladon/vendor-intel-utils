...
ctrlsys_handle_message(unsigned char *buf, unsigned int size) {
    ...
    if(msg->subtype > CTRL_MSGSYS_SUBTYPE_MAX) {
        ga_logger(Severity::INFO, "system-message: unknown subtype (%02x)\n", msg->subtype);
        return 1;
    }
...
#ifdef WIN32
ctrlmsg_t *
ctrlsys_netreport_delay(ctrlmsg_t *msg, double first, double last, double bw) {
    ctrlmsg_system_netreport_delay_t *msgn = (ctrlmsg_system_netreport_delay_t*)msg;
    bzero(msg, sizeof(ctrlmsg_system_netreport_delay_t));
    msgn->msgsize = htons(sizeof(ctrlmsg_system_netreport_delay_t));
    msgn->msgtype = CTRL_MSGTYPE_SYSTEM;
    msgn->subtype = CTRL_MSGSYS_SUBTYPE_NETREPORT;
    msgn->magic = htonl(0xde1a);
    msgn->first_delay = htonl((unsigned int)first);
    msgn->last_delay = htonl((unsigned int)last);
    msgn->estimated_bw = htonl((unsigned int)(bw * 1000.0 * 8));
    msgn->frame_number = htonl(3);
    return msg;
}
#endif
