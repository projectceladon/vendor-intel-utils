...
ga_avcodec_vencoder_init(AVCodecContext *ctx, AVCodec *codec, int width, int height, int fps, vector<string> *vso) {
    ...
    if(vso != NULL) {
        ...
        for(i = 0; i < n; i += 2) {
            ...
            ga_logger(Severity::INFO, "vencoder-init: option %s = %s\n",
                (*vso)[i].c_str(),
                (*vso)[i+1].c_str());
        }
    } else {
        ga_logger(Severity::INFO, "vencoder-init: using default video encoder parameter.\n");
    }

    std::lock_guard<std::mutex> lock(avcodec_open_mutex);
    if(avcodec_open2(ctx, codec, &opts) != 0) {
        avcodec_close(ctx);
        av_free(ctx);
        ga_logger(Severity::ERR, "vencoder-init: Failed to initialize encoder for codec \"%s\"\n", codec->name);
        return NULL;
    }
